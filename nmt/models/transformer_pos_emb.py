...
class Encoder(nn.Module):
    def __init__(self, input_dim, hid_dim, num_layers, num_heads,
...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        Q = self.fc_q(query)  # Q: [batch_size, query_len, hid_dim]
        K = self.fc_k(key)  # K: [batch_size, key_len, hid_dim]
        V = self.fc_v(value)  # V: [batch_size, value_len, hid_dim]
        Q = Q.view(batch_size, -1, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        K = K.view(batch_size, -1, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        V = V.view(batch_size, -1, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        # Q: [batch_size, num_heads, query_len, head_dim]
        # K: [batch_size, num_heads, key_len, head_dim]
        # V: [batch_size, num_heads, value_len, head_dim]
...
class DecoderLayer(nn.Module):
    def __init__(self, hid_dim, num_heads, pf_dim, dropout, device):
        super().__init__()
        self.self_attn_layer_norm = nn.LayerNorm(hid_dim)
...
class Decoder(nn.Module):
    def __init__(self, output_dim, hid_dim, num_layers, num_heads,
                 pf_dim, dropout, device, max_length=100):
        super().__init__()
        self.device = device
        ...
        self.pos_embedding = nn.Embedding(max_length, hid_dim)
        self.layers = nn.ModuleList(
            [DecoderLayer(hid_dim, num_heads, pf_dim, dropout, device) for _ in range(num_layers)]
        )
